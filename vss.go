...
func Verify_Share(peer_number string, N int64, T int64, signing bool) kyber.Scalar {
	var i int64
	path := ""
	path2 := ""
	if signing {
...
func Verify_Share_sign(peer_number string, N int64, T int64) kyber.Scalar {
	...
	// }
	verify_each_share_sign(peer_number, N, share, T)
...
//verify_each_share
func verify_each_share_sign(peer_number string, peer_count int64, share []kyber.Scalar, T int64) {
	var i int64
	fmt.Printf("Verifying shares Recieved By %s \n", peer_number)
	for i = 0; i <= peer_count; i++ {
		if i == int64(my_index) {
